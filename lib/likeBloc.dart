...
class DeleteSuccess extends LikeManagementState {}
class DeleteError extends LikeManagementState {}
...
Future<void> _onDeleteLike(event, emit) async {
  int id = (event as DeleteLikeEvent).id;
  final user = FirebaseAuth.instance.currentUser;

  if(user != null){
    CollectionReference likesRef = FirebaseFirestore.instance.collection('users').doc(user.uid).collection('likes');
